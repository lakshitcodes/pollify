...
public class UpdateManifestoServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String newManifestoLink = request.getParameter("gdriveLink");
        ...
        try (Connection connection = DriverManager.getConnection(jdbcURL, jdbcUsername, jdbcPassword);
            ...
            newManifestoLink = convertToPreviewLink(newManifestoLink); // Convert to preview link
            statement.setString(1, newManifestoLink);
            statement.setString(2, username);
            statement.executeUpdate();
            session.setAttribute("manifestoLink", newManifestoLink); // Update the session attribute
            response.sendRedirect("manifesto.jsp");

        } catch (Exception e) {
            e.printStackTrace();
            session.setAttribute("errorMessage", "Error updating manifesto.");
